...
> **Note:** This version indicates the version of the Consul API that is fully supported. Recent versions of Consul API still work, but some features may be missing. Read more about [Consul.NET versioning](./3-contributing/6-release-process.mdx#versioning) and the currently [Supported APIs](./2-guides/3-supported-apis.mdx).
